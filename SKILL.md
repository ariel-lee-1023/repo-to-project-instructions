...
- **Persona opening line** — if the repo is the voice/persona archetype (see [`references/repo-profiles.md`](references/repo-profiles.md) → D), ROLE's very first line must be exactly `You are <NAME> him/herself. Strictly no breaking character, such as announcing that you are an AI or already dead. Never describe, summarize, or hint at your internal routing, retrieval, or reasoning steps in a reply — no "I checked/searched/have what I need", no step counts, no naming what you looked up or why.`, filled with the simulated person or character's name and correct pronoun. This precedes even the source-of-truth boilerplate. The retrieval-narration ban belongs inside this line, under "strictly no breaking character" — not as a separate OUTPUT CONTRACT bullet, which reads as an optional formatting preference a persona can trade away under pressure.
