...
1. **Persistent instructions** — a text field applied to every turn in the Gem/Project. Present on repo-linked and local-file hosts. Absent entirely on session-based agents (Gemini Spark) — that absence is what makes session-opening mode its own path rather than a thinner version of the other two.
...
3. **Fetch at answer time** — web access, or a persisted/attached link back to a repo. The least reliable of the three on hosts that have it: available, then unavailable, then available again, sometimes silently. Some hosts (Gemini Gem, as of this writing) do not offer this capability at all for a git repo — see below. Others offer it only within a single conversation with nothing persisted (Gemini Spark) — also see below.

For hosts with (1): the compiled instructions are pasted into it and are designed around (2) and (3) being where the source's actual content comes from where they exist. They must still degrade usefully when only (1) is working — a thinner answer that names what it could not reach, never a confident one built from nothing. For a host with (3) but no (1) — session-opening mode — the compiled message carries a compressed version of everything (1) would normally hold, re-sent every session, because there is nowhere for it to persist between sessions.
...
- **Gemini Spark is a different product, not a Gem with a new name — and it is in scope, but for a different mode.** Spark is a session-based agent (Workspace, MCP-based third-party apps, and local files on macOS) with no persistent Instructions field and no Knowledge upload area the way a Gem has. Do not compile Gem-style instructions for it and do not assume anything said here about Gem Knowledge applies to it. What Spark *does* do, confirmed by direct testing: within a single conversation, it reads a repo attached to a chat turn as a link/card and retrieves from it there. That is exactly the capability session-opening mode is built around — see `## Gemini Spark` below.

## Gemini Spark

Spark has no Instructions field and no Knowledge upload area — there is nothing persistent to paste into, and this skill does not produce a file meant to be pasted once and left. What it does have: a chat turn can carry an attached repo (as a link/card, not necessarily a bare pasted URL) alongside a text message, and Spark fetches and reads that repo within the conversation it was attached to. This is session-opening mode's entire premise, confirmed by direct testing rather than assumed from the product description.

- **Nothing carries over between sessions.** Every new Spark conversation starts from zero — no memory of a previous session's routing rules, invariants, or role framing unless the user re-sends them. The message from Step 5 is designed to be sent fresh, attached to the repo, at the start of every session that should use this source — not once, ever.
- **The opening message's first clause drives the session's auto-generated title.** Left to a generic opener ("你会用这个吗", a bare attachment with no accompanying text), Spark names the session after the *interaction* ("Gemini Spark 功能评估") rather than the repo's actual subject. A subject-bearing opening clause is not a nice-to-have in this mode — it is most of why session-opening mode exists as separate from just attaching a link and saying nothing.
- **Attach the repo the way the product surfaces it, not as a raw pasted URL if a native attach affordance exists.** The tested behavior used an attached repo card; a bare URL pasted into text may or may not fetch the same way, and this skill cannot verify which without checking the current UI at build time.
- **There is no citation display to check routing against, the way a Gem or Project has.** Verifying the fetch happened is closer to asking directly after the opening turn, or checking whether the reply's content could only have come from the repo (a specific rubric detail, a specific file name) rather than from the model's general knowledge of the topic.
- **Do not compile the main instruction template for this host.** If a user asks for "Gem instructions" or "Project instructions" for a Spark session, that is very likely a language slip inherited from Gem-era habits carried into a still-unfamiliar product, not an actual request for a persistent field Spark does not have — confirm which mode they mean before compiling the wrong artifact.
- Setup to hand the user: attach the repo link/card to a new Spark conversation → send the compiled opening message from Step 5 in the same turn → check the auto-generated session title matches the repo's subject → repeat this exact pairing (link + message) at the start of every future session meant to use this source.
